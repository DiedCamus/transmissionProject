...
def index():
    ...
    return render_template('downloadIndex.html',filenames=filenames, the_message=the_message, USERNAME=current_username)
...
def page_get():
    ...
    filenames = get_allfilename()
    the_message = u'下载成功！'
    return render_template('downloadIndex.html', filenames=filenames, the_message=the_message, USERNAME=app.current_username)
...
def show_logs():
	if not session.get('logged_in'):
		return redirect(url_for('login'))
	fp = open("downloadlog.log", "r")
	lines = fp.readlines()
	return render_template('downloadlog.html',logs=lines)
